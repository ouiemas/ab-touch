...
export default function About() {
  ...
  return (
    <section className="relative w-full bg-[#F8F4EC] text-center lg:text-left py-16 md:py-24 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative z-10">
        ...
        >
          {/* Left Content */}
          <div className="w-full lg:w-[55%] space-y-6 flex flex-col items-center lg:items-start">
            <motion.span 
              variants={fadeIn}
            ...
            <motion.h2 
              variants={fadeIn}
              className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight leading-tight text-slate-900"
            >
              Une agence créative <br className="hidden sm:block" />
            ...
            <motion.p 
              variants={fadeIn}
              className="text-slate-700 text-sm sm:text-base md:text-lg leading-relaxed max-w-2xl font-normal text-center lg:text-left px-1 sm:px-0"
            >
            ...
            {/* Core Pillars */}
            <motion.div variants={containerVariants} className="mt-6 space-y-5 w-full text-left">
              {[
                ...
                { icon: Star, title: "Nos valeurs", text: "Excellence operational • Créativité disruptive • Innovation continue • Transparence" }
              ].map((item, idx) => (
                <motion.div 
                  key={idx} 
                  variants={fadeIn}
                  className="flex gap-4 items-start group bg-white/40 lg:bg-transparent p-3 sm:p-0 rounded-xl"
                >
                  ...
                  <div className="space-y-0.5">
                    <h4 className="font-semibold text-base sm:text-lg text-slate-900">{item.title}</h4>
          ...
          <motion.div 
            variants={fadeIn}
            className="w-full sm:max-w-md lg:max-w-none lg:w-[40%] relative shrink-0 mt-6 lg:mt-0"
          >
            {/* Elegant, high-end accent background positioning */}
            <motion.div 
              animate={{ y: [0, -6, 0] }}
              transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
              className="absolute inset-0 bg-[#109DA2]/5 rounded-2xl transform translate-x-2 translate-y-2 sm:translate-x-3 sm:translate-y-3 -z-10 border border-[#109DA2]/10" 
            />
            <div className="overflow-hidden rounded-2xl shadow-xl border border-slate-200 bg-slate-100 w-full h-[280px] sm:h-[350px] lg:h-[480px]">
              <img
                src="/her.png" 
        ...
        <motion.div 
          ...
          viewport={{ once: true, margin: "-100px" }}
          variants={containerVariants}
          className="bg-[#109DA2] rounded-2xl mt-16 md:mt-28 text-white shadow-xl overflow-hidden w-full"
        >
          {/* تم تعديل الـ Grid هنا لتصبح متجاوبة بشكل كامل على الهواتف بدون تداخل الخطوط الحواف */}
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-white/10 border-t border-b border-white/5">
            {[
              ...
              <motion.div 
                key={i} 
                variants={fadeIn}
                className="text-center py-8 sm:py-14 px-4 sm:px-6 space-y-1.5"
              >
                <stat.icon size={20} className="mx-auto mb-2 sm:mb-4 text-[#D9B15A] opacity-95" />
                <div className="text-2xl sm:text-4xl font-bold tracking-tight">{stat.val}</div>
                <p className="text-white/80 font-medium text-[10px] sm:text-xs md:text-sm tracking-wide uppercase px-1">{stat.lbl}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        <div className="mt-20 md:mt-32 w-full">
          ...
          >
            <h3 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-4xl">Pourquoi nous choisir ?</h3>
            <p className="text-slate-500 max-w-xl mx-auto text-xs sm:text-sm md:text-base px-2">L'assurance d'un savoir-faire premium adapté à vos exigences de performance.</p>
          ...
          <motion.div 
            ...
            variants={containerVariants}
            className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 sm:gap-6 mt-12 sm:mt-16 text-left"
          >
            {[
              ...
              <motion.div
                ...
                className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 sm:p-8 transition-all duration-300 group"
              >
                <div className="bg-[#109DA2]/5 w-10 h-10 rounded-lg flex items-center justify-center mb-4 sm:mb-6 transition-colors duration-300 group-hover:bg-[#109DA2]">
                  <ArrowRight className="text-[#109DA2] transition-colors duration-300 group-hover:text-white" size={16} />
                </div>
                <h4 className="font-semibold text-base sm:text-lg text-slate-900 mb-2">{item.title}</h4>
                <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">{item.text}</p>
              </motion.div>
            ))}
          </motion.div>
        ...
        <motion.div 
          ...
          className="mt-20 md:mt-32 bg-[#14201E] rounded-2xl text-white p-8 sm:p-12 md:p-16 flex flex-col lg:flex-row justify-between items-center gap-6 sm:gap-8 shadow-xl w-full relative overflow-hidden border border-slate-800 text-center lg:text-left"
        >
          ...
          <div className="max-w-xl space-y-2.5">
            <h3 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight">Vous avez un projet en perspective ?</h3>
          ...
          >
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="bg-[#109DA2] text-white font-semibold px-7 py-3.5 rounded-lg shadow-md transition-all duration-200 shrink-0 tracking-wide text-xs sm:text-sm w-full sm:w-auto"
            >
              Demander un devis
            </motion.button>
          </a>
